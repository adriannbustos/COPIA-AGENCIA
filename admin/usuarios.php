...
// ==================== ESTANDARIZAR COLUMNA ROL ====================
// Usar 'rol' para coincidir con crear_admin_definitivo.php
$columna_rol = 'rol';
try {
$columns = $conn->query("SHOW COLUMNS FROM usuarios")->fetchAll(PDO::FETCH_COLUMN);
// Si existe 'role' pero no 'rol', renombrar
...
logAuditoria($conn, 'columna_renombrada', 'usuarios', null, ['de' => 'role', 'a' => 'rol']);
}
// Si no existe ninguna, crear 'rol'
...
logAuditoria($conn, 'columna_rol_creada', 'usuarios', null, ['columna' => 'rol']);
}
// ✅ CORRECCIÓN CRÍTICA: Forzar columna 'rol' a VARCHAR(50) para evitar truncamiento con nuevos roles
else {
$stmt_check_rol = $conn->query("DESCRIBE usuarios rol");
$rol_info = $stmt_check_rol->fetch(PDO::FETCH_ASSOC);
if ($rol_info) {
// Si es ENUM o VARCHAR con longitud < 50, modificar a VARCHAR(50)
if (stripos($rol_info['Type'], 'enum') !== false || 
    (stripos($rol_info['Type'], 'varchar') !== false && preg_match('/varchar\((\d+)\)/i', $rol_info['Type'], $matches) && isset($matches[1]) && $matches[1] < 50)) {
$conn->exec("ALTER TABLE usuarios MODIFY COLUMN rol VARCHAR(50) NOT NULL DEFAULT 'operador'");
logAuditoria($conn, 'columna_rol_actualizada_varchar50', 'usuarios', null, ['anterior' => $rol_info['Type'], 'nuevo' => 'VARCHAR(50)']);
}
}
}
// ✅ AGREGAR: Verificar columna nombre_completo
...
logAuditoria($conn, 'columna_nombre_completo_creada', 'usuarios', null, ['columna' => 'nombre_completo']);
}
// ✅ AGREGAR: Verificar columna jurisdiccion
...
logAuditoria($conn, 'columna_jurisdiccion_creada', 'usuarios', null, ['columna' => 'jurisdiccion']);
}
...
// ✅ CORRECCIÓN: Verificar que la columna password tenga longitud suficiente para bcrypt
...
logAuditoria($conn, 'columna_password_ampliada', 'usuarios', null, ['anterior' => $pass_info['Type'], 'nuevo' => 'VARCHAR(255)']);
}
}
...
// ==================== GENERAR CREDENCIAL DE USUARIO (PDF CON QR) ====================
if (isset($_GET['action']) && $_GET['action'] === 'generar_credencial_usuario') {
require_once '../config/auth.php';
...
if (!$usuario_id) {
die('<h2 style="color:red;text-align:center;padding:50px">❌ ID de usuario no válido</h2>');
}
try {
$stmt = $conn->prepare("
...
if (!$usuario) {
die('<h2 style="color:red;text-align:center;padding:50px">❌ Usuario no encontrado</h2>');
}
} catch(PDOException $e) {
die('<h2 style="color:red;text-align:center;padding:50px">❌ Error al cargar los datos del usuario</h2>');
}
...
$foto_path = '../uploads/fotos_personal/' . ($usuario['foto'] ?? '');
...
$pdf->SetXY(4, 47);
// ✅ AQUÍ SE COLOCA EL NOMBRE DE USUARIO EN LUGAR DEL ID
$pdf->Cell(28, 6, strtoupper($usuario['username']), 0, 0, 'C');
$pdf->SetFont('Arial', 'B', 9);
$pdf->SetXY(34, 12);
$pdf->Cell(50, 5, 'Rol: ' . strtoupper(utf8_decode($usuario['rol'] ?? 'USUARIO')), 0, 0, 'L');
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY(34, 17);
$pdf->Cell(50, 5, 'Nombre: ' . strtoupper(utf8_decode($usuario['nombre_completo'] ?? '')), 0, 0, 'L');
$pdf->SetXY(34, 22);
$pdf->Cell(50, 5, 'Email: ' . strtoupper(utf8_decode($usuario['email'] ?? '')), 0, 0, 'L');
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY(34, 27);
$pdf->Cell(50, 5, 'Empresa: ' . strtoupper(utf8_decode($usuario['empresa_nombre'] ?? 'SIN EMPRESA')), 0, 0, 'L');
...
$pdf->Image($escudo_path, $bloque_x, $bloque_y, $ancho_escudo, $ancho_escudo, 'PNG');
}
}
$firma_x = $bloque_x + $ancho_escudo + $gap;
$firma_path = '../uploads/firmas_jefe/firma_admin.png';
if (file_exists($firma_path)) {
$info = @getimagesize($firma_path);
if ($info !== false && $info[2] === IMAGETYPE_PNG) {
$pdf->Image($firma_path, $firma_x, $bloque_y, $ancho_firma, $alto_firma, 'PNG');
}
}
...
$pdf->Cell(85.6, $line_height, 'ADMINISTRADOR DEL SISTEMA', 0, 0, 'C');
$pdf->SetFont('Arial', 'B', 5);
$pdf->SetXY(25, $texto_y + $line_height);
$pdf->Cell(85.6, $line_height, 'JEFE DE ACCESO Y SEGURIDAD', 0, 0, 'C');
$pdf->SetFont('Arial', 'B', 4.5);
$pdf->SetXY(25, $texto_y + (2 * $line_height));
$pdf->Cell(85.6, $line_height, 'AREA DE GESTIÓN DE USUARIOS', 0, 0, 'C');
$pdf->AddPage('L', array(85.6, 54));
...
$payload_token = $usuario_id . '|' . $expiracion_timestamp . '|usuario';
...
$qr_temp = sys_get_temp_dir() . '/qr_usuario_' . $usuario_id . '_' . time() . '.png';
...
$pdf->SetXY(0, 42);
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(85.6, 5, 'AREA DE ACCESO Y SEGURIDAD', 0, 0, 'C');
$pdf->SetXY(0, 47);
$pdf->SetFont('Arial', '', 6);
...
$nombre_archivo = 'Credencial_Usuario_' . preg_replace('/[^a-zA-Z0-9_]/', '_', $usuario['username']) . '_' . $usuario_id . '.pdf';
$pdf->Output('D', $nombre_archivo);
exit;
}
// ==================== FIN GENERAR CREDENCIAL DE USUARIO ====================
// ==================== ROLES DISPONIBLES ====================
$roles_disponibles = [
...
'profesores' => ['nombre' => 'Profesores', 'icono' => 'fa-chalkboard-teacher', 'color' => '#16a085', 'descripcion' => 'Gestión académica y capacitación'],
'superadmin' => ['nombre' => 'Superadmin', 'icono' => 'fa-user-crown', 'color' => '#8e44ad', 'descripcion' => 'Administración maestra del sistema']
];
// ==================== JURISDICCIONES DISPONIBLES ====================
$jurisdicciones_disponibles = [
'Esquel' => 'Esquel',
'Comodoro' => 'Comodoro',
...
// ✅ CORRECCIÓN A6: Escapar caracteres especiales % y _ para LIKE
$search_username_escaped = addcslashes($search_username, '%_');
$where_clauses[] = "(username LIKE ? OR email LIKE ? OR nombre_completo LIKE ?)";
$params[] = '%' . $search_username_escaped . '%';
$params[] = '%' . $search_username_escaped . '%';
$params[] = '%' . $search_username_escaped . '%';
}
...
// ✅ AGREGAR: nombre_completo y jurisdiccion en el SELECT
$sql = "
SELECT u.id, u.username, u.nombre_completo, u.email, u.rol as role, u.ultimo_login, u.created_at, u.empresa_id, u.jurisdiccion,
...
$usuarios = [];
$error = "Error cargando usuarios: " . htmlspecialchars($e->getMessage());
error_log($e->getMessage());
}
// ==================== GUARDAR/ACTUALIZAR USUARIO ====================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_usuario'])) {
try {
// ✅ CORRECCIÓN M1: Validar token CSRF
if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
throw new Exception('Token CSRF inválido');
}
$username = trim($_POST['username'] ?? '');
// ✅ AGREGAR: nombre_completo y jurisdiccion
$nombre_completo = trim($_POST['nombre_completo'] ?? '');
$jurisdiccion = !empty($_POST['jurisdiccion']) ? trim($_POST['jurisdiccion']) : null;
$email = trim($_POST['email'] ?? '');
$role = trim($_POST['role'] ?? 'operador');
// ✅ CORRECCIÓN CRÍTICA: Obtener contraseña SIN sanitizeInput para preservar caracteres especiales
$password = $_POST['password'] ?? null;
$usuario_id = $_POST['usuario_id'] ?? null;
$empresa_id = !empty($_POST['empresa_id']) ? (int)$_POST['empresa_id'] : null;
// ✅ CORRECCIÓN M2: Eliminar logs con información sensible (comentados para producción)
/*
error_log("=== GUARDANDO USUARIO ===");
error_log("username: $username");
error_log("nombre_completo: $nombre_completo");
error_log("jurisdiccion: $jurisdiccion");
error_log("role: $role");
error_log("usuario_id: " . ($usuario_id ?? 'NULL'));
error_log("empresa_id: " . ($empresa_id ?? 'NULL'));
error_log("password_provided: " . (!empty($password) ? 'SI' : 'NO'));
*/
// Validar rol - Permitir cualquier valor alfanumérico de hasta 50 caracteres para nuevos roles
if (!preg_match('/^[a-zA-Z0-9_]{1,50}$/', $role)) {
...
// Validar empresa si el rol es empresa
if ($role === 'empresa' && empty($empresa_id)) {
throw new Exception('Debe seleccionar una empresa para este rol');
}
// Validar jurisdiccion si el rol es operador
if ($role === 'operador' && empty($jurisdiccion)) {
throw new Exception('Debe seleccionar una jurisdicción para este rol');
}
// Verificar username duplicado
$stmt = $conn->prepare("SELECT id FROM usuarios WHERE username = ? AND id != ?");
$stmt->execute([$username, $usuario_id ?? 0]);
if ($stmt->fetch() && !$usuario_id) {
throw new Exception('El nombre de usuario ya existe');
}
// Verificar email duplicado
$stmt = $conn->prepare("SELECT id FROM usuarios WHERE email = ? AND id != ?");
$stmt->execute([$email, $usuario_id ?? 0]);
if ($stmt->fetch() && !$usuario_id) {
...
if (!empty($password)) {
// ✅ CORRECCIÓN: Usar PASSWORD_DEFAULT para compatibilidad futura y verificar longitud
$hashed_password = password_hash($password, PASSWORD_DEFAULT);
// Verificar que el hash se generó correctamente
if (!$hashed_password || strlen($hashed_password) > 255) {
error_log("ERROR: Hash de contraseña inválido para usuario ID: $usuario_id");
...
// ✅ CORRECCIÓN M2: Eliminar log sensible
// error_log("Usuario actualizado CON contraseña - ID: $usuario_id, hash_len: " . strlen($hashed_password));
} else {
$stmt = $conn->prepare("
UPDATE usuarios SET
username = ?,
...
// ✅ CORRECCIÓN M2: Eliminar log sensible
// error_log("Usuario actualizado SIN contraseña - ID: $usuario_id");
}
logAuditoria($conn, 'usuario_actualizado', 'usuarios', $usuario_id, [
'username' => $username,
...
// ✅ CORRECCIÓN: Usar PASSWORD_DEFAULT y verificar que el hash sea válido
$hashed_password = password_hash($password, PASSWORD_DEFAULT);
if (!$hashed_password || strlen($hashed_password) > 255) {
error_log("ERROR: Hash de contraseña inválido para nuevo usuario: $username");
throw new Exception('Error al procesar la contraseña');
}
$stmt = $conn->prepare("
INSERT INTO usuarios (username, nombre_completo, email, password, rol, empresa_id, jurisdiccion, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
");
$stmt->execute([$username, $nombre_completo, $email, $hashed_password, $role, $empresa_id, $jurisdiccion]);
$nuevo_id = $conn->lastInsertId();
// ✅ CORRECCIÓN M2: Eliminar log sensible
// error_log("Usuario creado - ID: $nuevo_id, Rol: $role, Jurisdiccion: $jurisdiccion, hash_len: " . strlen($hashed_password));
logAuditoria($conn, 'usuario_creado', 'usuarios', $nuevo_id, [
'username' => $username,
'nombre_completo' => $nombre_completo,
'role' => $role,
'empresa_id' => $empresa_id,
...
try {
// ✅ CORRECCIÓN M1: Validar token CSRF en eliminación
if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
...
$stmt->execute([$id]);
logAuditoria($conn, 'usuario_eliminado', 'usuarios', $id, ['id' => $id]);
...
// Funciones para URL y ordenamiento
...
.table-modern { margin-bottom: 0; min-width: 1200px; }
.table-modern thead {
background: linear-gradient(135deg, #4361ee, #3a0ca3);
color: white;
}
...
.role-profesores { background: #16a085; color: white; }
.role-superadmin { background: #8e44ad; color: white; }
/* Pagination */
.pagination-modern {
display: flex;
justify-content: center;
...
/* Modal */
.modal-edit-responsive .modal-content {
border: none;
border-radius: 24px;
background: linear-gradient(135deg, #ffffff 0%, #f8f9ff 100%);
box-shadow: 0 25px 80px rgba(67, 97, 238, 0.25);
}
.modal-edit-responsive .modal-header {
background: linear-gradient(135deg, #4361ee 0%, #3a0ca3 100%);
padding: 25px 30px;
border: none;
border-radius: 24px 24px 0 0;
}
.modal-edit-responsive .modal-title {
color: white;
font-weight: 700;
font-size: 1.3rem;
display: flex;
...
.modal-edit-responsive .btn-close-white {
...
.modal-edit-responsive .btn-close-white:hover {
background: rgba(255, 255, 255, 0.35);
transform: rotate(90deg);
}
.modal-edit-responsive .modal-body {
padding: 35px 30px;
max-height: 70vh;
overflow-y: auto;
}
.modal-edit-responsive .form-section {
background: white;
border-radius: 18px;
padding: 25px;
margin-bottom: 20px;
...
.modal-edit-responsive .section-header {
display: flex;
align-items: center;
gap: 10px;
margin-bottom: 20px;
...
.modal-edit-responsive .section-header i {
color: #4361ee;
width: 35px;
height: 35px;
...
.modal-edit-responsive .section-header span {
font-weight: 700;
color: #2c3e50;
}
.modal-edit-responsive .form-label {
font-weight: 600;
color: #4a5568;
margin-bottom: 8px;
display: flex;
...
.modal-edit-responsive .form-control,
.modal-edit-responsive .form-select {
border: 2px solid #e2e8f0;
border-radius: 12px;
padding: 12px 16px;
font-size: 0.95rem;
...
.modal-edit-responsive .form-control:focus,
.modal-edit-responsive .form-select:focus {
border-color: #4361ee;
box-shadow: 0 0 0 4px rgba(67, 97, 238, 0.15);
outline: none;
}
.modal-edit-responsive .form-info {
display: flex;
align-items: center;
gap: 8px;
color: #718096;
...
.modal-edit-responsive .form-actions {
display: flex;
gap: 12px;
}
.modal-edit-responsive .btn-cancel {
background: linear-gradient(135deg, #f8f9fa, #e9ecef);
color: #4a5568;
border: 2px solid #e2e8f0;
padding: 12px 28px;
...
.modal-edit-responsive .btn-cancel:hover {
background: linear-gradient(135deg, #e9ecef, #dee2e6);
border-color: #cbd5e0;
transform: translateY(-2px);
}
.modal-edit-responsive .btn-save {
background: linear-gradient(135deg, #4361ee, #3a0ca3);
color: white;
border: none;
padding: 12px 28px;
...
.modal-edit-responsive .btn-save:hover {
transform: translateY(-3px);
box-shadow: 0 10px 30px rgba(67, 97, 238, 0.5);
}
.empresa-section {
...
.empresa-section.active {
display: block;
animation: fadeIn 0.3s ease;
}
.jurisdiccion-section {
display: none;
...
.stats-container-modern { grid-template-columns: repeat(2, 1fr); gap: 15px; }
}
...
.filter-header-modern { flex-direction: column; align-items: flex-start; }
.filter-actions-modern { width: 100%; justify-content: stretch; flex-direction: column; }
.table-modern { font-size: 0.75rem; min-width: 1050px; }
.modal-edit-responsive .modal-dialog { margin: 5px; max-width: calc(100% - 10px); }
.modal-edit-responsive .modal-footer { flex-direction: column; gap: 15px; }
.modal-edit-responsive .form-actions { width: 100%; justify-content: stretch; flex-direction: column; }
.modal-edit-responsive .btn-cancel,
.modal-edit-responsive .btn-save { width: 100%; justify-content: center; }
}
...
<div class="stat-card-modern">
<div style="width:70px;height:70px;border-radius:22px;background:linear-gradient(135deg,#e74c3c,#c0392b);color:white;display:flex;align-items:center;justify-content:center;margin-bottom:20px;font-size:1.9rem;">
<i class="fas fa-user-shield"></i>
</div>
<div style="font-size:2.8rem;font-weight:800;margin:10px 0;color:#2c3e50;">
<?php echo count(array_filter($usuarios, fn($u) => ($u['role'] ?? 'operador') == 'administrador')); ?>
</div>
<div style="font-size:1.25rem;font-weight:700;color:#2c3e50;margin-top:5px;">Administradores</div>
</div>
<div class="stat-card-modern">
<div style="width:70px;height:70px;border-radius:22px;background:linear-gradient(135deg,#27ae60,#2ecc71);color:white;display:flex;align-items:center;justify-content:center;margin-bottom:20px;font-size:1.9rem;">
<i class="fas fa-building"></i>
</div>
<div style="font-size:2.8rem;font-weight:800;margin:10px 0;color:#2c3e50;">
<?php echo count(array_filter($usuarios, fn($u) => ($u['role'] ?? 'operador') == 'empresa')); ?>
</div>
<div style="font-size:1.25rem;font-weight:700;color:#2c3e50;margin-top:5px;">Usuarios Empresa</div>
</div>
<div class="stat-card-modern">
<div style="width:70px;height:70px;border-radius:22px;background:linear-gradient(135deg,#3498db,#2980b9);color:white;display:flex;align-items:center;justify-content:center;margin-bottom:20px;font-size:1.9rem;">
<i class="fas fa-user-cog"></i>
</div>
<div style="font-size:2.8rem;font-weight:800;margin:10px 0;color:#2c3e50;">
<?php echo count(array_filter($usuarios, fn($u) => in_array($u['role'] ?? 'operador', ['operador', 'auditor', 'usuario']))); ?>
</div>
<div style="font-size:1.25rem;font-weight:700;color:#2c3e50;margin-top:5px;">Operadores</div>
</div>
</div>
<!-- FILTROS -->
...
<div class="filter-badge-modern" style="display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#4361ee,#4cc9f0);color:white;padding:8px 18px;border-radius:25px;font-size:0.85rem;font-weight:700;">
<i class="fas fa-check-circle"></i><span>Filtros Activos</span>
</div>
<?php endif; ?>
</div>
...
<option value="<?php echo $role_key; ?>" <?php echo ($search_role === $role_key) ? 'selected' : ''; ?>>
...
<option value="<?php echo $emp['id']; ?>" <?php echo ($search_empresa == $emp['id']) ? 'selected' : ''; ?>>
...
<div style="display:flex;align-items:center;gap:10px;padding:12px 18px;background:linear-gradient(135deg,#ebf4ff,#e6f0ff);border-radius:12px;border:1px solid rgba(67,97,238,0.2);font-size:0.9rem;color:#4361ee;font-weight:600;" class="w-100">
<i class="fas fa-database"></i><span><?php echo $total_registros; ?> registros</span>
</div>
</div>
</div>
...
<h3 class="mb-3">No hay usuarios registrados</h3>
<p class="mb-4"><?php echo empty($search_username) && $search_role === 'todos' && empty($search_empresa) ? 'Registra tu primer usuario para comenzar.' : 'No se encontraron usuarios con los filtros aplicados.'; ?></p>
<button class="btn btn-success btn-lg" type="button" data-bs-toggle="modal" data-bs-target="#nuevoUsuarioModal">
<i class="fas fa-plus me-2"></i><?php echo empty($search_username) && $search_role === 'todos' && empty($search_empresa) ? 'Crear Primer Usuario' : 'Crear Nuevo Usuario'; ?>
</button>
</div>
<?php else: ?>
...
<th>Jurisdicción</th>
<th>Empresa</th>
<th><a href="<?php echo generarUrlOrden('ultimo_login', $orden_columna, $orden_direccion); ?>" class="text-white text-decoration-none">Último Login <?php echo mostrarIconoOrden('ultimo_login', $orden_columna, $orden_direccion); ?></a></th>
<th>Acciones</th>
</tr>
</thead>
<tbody>
...
$role_info = $roles_disponibles[$role] ?? ['nombre' => ucfirst($role), 'icono' => 'fa-user', 'color' => '#6c757d', 'descripcion' => 'Rol personalizado'];
?>
<span class="role-badge role-<?php echo preg_replace('/[^a-zA-Z0-9_-]/', '', $role); ?>">
<i class="fas <?php echo $role_info['icono']; ?>"></i>
<?php echo htmlspecialchars($role_info['nombre']); ?>
</span>
</td>
<td>
...
if ($ultimo_login && $ultimo_login != '0000-00-00 00:00:00' && $ultimo_login != '1970-01-01 00:00:01') {
echo '<i class="fas fa-clock text-muted me-1"></i>' . date('d/m/Y H:i', strtotime($ultimo_login));
} else {
echo '<span class="text-muted"><i class="fas fa-times-circle me-1"></i>Nunca</span>';
}
?>
</td>
<td>
<div class="d-flex gap-1">
<button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editarUsuarioModal<?php echo $u['id']; ?>" title="Editar">
<i class="fas fa-edit"></i>
</button>
<?php if ($u['id'] != $user['id']): ?>
<button type="button" class="btn btn-sm btn-outline-danger eliminar-usuario"
data-id="<?php echo $u['id']; ?>" data-username="<?php echo htmlspecialchars($u['username']); ?>" title="Eliminar">
<i class="fas fa-trash"></i>
</button>
<?php endif; ?>
<!-- ✅ NUEVO BOTÓN: GENERAR CREDENCIAL DE USUARIO -->
<a href="usuarios.php?action=generar_credencial_usuario&id=<?php echo $u['id']; ?>"
class="btn btn-sm btn-info" target="_blank" title="Generar Credencial">
...
<input type="text" name="nombre_completo" class="form-control" placeholder="Ej: Juan Pérez">
...
<select name="role" class="form-select" required id="roleNuevo">
<?php foreach ($roles_disponibles as $role_key => $role_info): ?>
<option value="<?php echo $role_key; ?>" data-descripcion="<?php echo $role_info['descripcion']; ?>">
<?php echo $role_info['nombre']; ?>
</option>
<?php endforeach; ?>
</select>
...
<span id="roleDescripcionNuevo">Acceso limitado</span>
</div>
</div>
</div>
</div>
<!-- Sección Jurisdicción (solo para rol operador) -->
<div class="jurisdiccion-section" id="jurisdiccionSectionNuevo">
<div class="section-header">
<i class="fas fa-map-marker-alt"></i>
<span>Jurisdicción Asignada</span>
</div>
<div class="row g-3">
<div class="col-md-12">
<label class="form-label">
...
<?php foreach ($jurisdicciones_disponibles as $jur_key => $jur_nombre): ?>
<option value="<?php echo $jur_key; ?>">
<?php echo htmlspecialchars($jur_nombre); ?>
</option>
<?php endforeach; ?>
</select>
<div class="form-text">
<i class="fas fa-info-circle text-primary"></i>
El usuario operará en esta jurisdicción
</div>
</div>
</div>
</div>
<!-- Sección Empresa (solo para rol empresa) -->
...
<?php echo htmlspecialchars($emp['nombre']); ?>
<?php if (!empty($emp['nombre_comercial'])): ?>
...
El usuario podrá gestionar esta empresa
</div>
</div>
</div>
</div>
...
<!-- MODALES DE EDICIÓN (uno por usuario) -->
<?php foreach ($usuarios as $u): ?>
...
<span>Editar Usuario: <?php echo htmlspecialchars($u['username']); ?></span>
</h5>
<button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
...
value="<?php echo htmlspecialchars($u['username']); ?>" placeholder="Ej: jperez">
</div>
<div class="col-md-6">
<label class="form-label">
<i class="fas fa-id-card"></i>
...
value="<?php echo htmlspecialchars($u['nombre_completo'] ?? ''); ?>" placeholder="Ej: Juan Pérez">
<div class="form-text">Opcional</div>
</div>
<div class="col-md-6">
<label class="form-label">
...
value="<?php echo htmlspecialchars($u['email']); ?>" placeholder="usuario@empresa.com">
</div>
<div class="col-md-6">
<label class="form-label">
<i class="fas fa-lock"></i>
...
<select name="role" class="form-select" required id="roleEdit<?php echo $u['id']; ?>">
<?php foreach ($roles_disponibles as $role_key => $role_info): ?>
<option value="<?php echo $role_key; ?>"
<?php echo ($u['role'] ?? 'operador') == $role_key ? 'selected' : ''; ?>
data-descripcion="<?php echo $role_info['descripcion']; ?>">
<?php echo $role_info['nombre']; ?>
</option>
<?php endforeach; ?>
</select>
<div class="form-text mt-2">
<i class="fas fa-info-circle text-primary"></i>
<span id="roleDescripcionEdit<?php echo $u['id']; ?>">
<?php
$role_actual = $u['role'] ?? 'operador';
echo ($roles_disponibles[$role_actual]['descripcion'] ?? 'Rol personalizado');
?>
</span>
</div>
</div>
...
<!-- Sección Jurisdicción (solo para rol operador) -->
<div class="jurisdiccion-section <?php echo ($u['role'] ?? 'operador') == 'operador' ? 'active' : ''; ?>"
id="jurisdiccionSectionEdit<?php echo $u['id']; ?>">
<div class="section-header">
<i class="fas fa-map-marker-alt"></i>
<span>Jurisdicción Asignada</span>
</div>
...
<?php foreach ($jurisdicciones_disponibles as $jur_key => $jur_nombre): ?>
<option value="<?php echo $jur_key; ?>"
<?php echo ($u['jurisdiccion'] ?? '') == $jur_key ? 'selected' : ''; ?>>
<?php echo htmlspecialchars($jur_nombre); ?>
</option>
<?php endforeach; ?>
</select>
...
<!-- Sección Empresa (solo para rol empresa) -->
<div class="empresa-section <?php echo ($u['role'] ?? 'operador') == 'empresa' ? 'active' : ''; ?>"
id="empresaSectionEdit<?php echo $u['id']; ?>">
<div class="section-header">
<i class="fas fa-building"></i>
<span>Empresa Asignada</span>
</div>
...
<?php foreach ($empresas_disponibles as $emp): ?>
<option value="<?php echo $emp['id']; ?>"
<?php echo ($u['empresa_id'] ?? 0) == $emp['id'] ? 'selected' : ''; ?>>
<?php echo htmlspecialchars($emp['nombre']); ?>
<?php if (!empty($emp['nombre_comercial'])): ?>
(<?php echo htmlspecialchars($emp['nombre_comercial']); ?>)
<?php endif; ?>
...
<div class="modal-body">
<p>¿Eliminar al usuario <strong id="usernameEliminar"></strong>?</p>
<p class="text-danger"><i class="fas fa-exclamation-circle"></i> Esta acción no se puede deshacer.</p>
</div>
<div class="modal-footer">
<!-- ✅ CORRECCIÓN M1: Token CSRF en modal de eliminación -->
...
// Mostrar/ocultar sección jurisdicción según rol seleccionado (Nuevo)
const roleNuevo = document.getElementById('roleNuevo');
const jurisdiccionSectionNuevo = document.getElementById('jurisdiccionSectionNuevo');
const empresaSectionNuevo = document.getElementById('empresaSectionNuevo');
const roleDescripcionNuevo = document.getElementById('roleDescripcionNuevo');
if (roleNuevo) {
...
// Toggle jurisdiccion section
if (esOperador) {
jurisdiccionSectionNuevo.classList.add('active');
jurisdiccionSectionNuevo.querySelector('select').setAttribute('required', 'required');
} else {
jurisdiccionSectionNuevo.classList.remove('active');
jurisdiccionSectionNuevo.querySelector('select').removeAttribute('required');
}
// Toggle empresa section
if (esEmpresa) {
empresaSectionNuevo.classList.add('active');
empresaSectionNuevo.querySelector('select').setAttribute('required', 'required');
} else {
empresaSectionNuevo.classList.remove('active');
empresaSectionNuevo.querySelector('select').removeAttribute('required');
}
roleDescripcionNuevo.textContent = selectedOption.dataset.descripcion || 'Acceso limitado';
});
// Trigger inicial
roleNuevo.dispatchEvent(new Event('change'));
}
// Mostrar/ocultar sección jurisdicción y empresa para cada modal de edición
<?php foreach ($usuarios as $u): ?>
(function() {
const roleEdit = document.getElementById('roleEdit<?php echo $u['id']; ?>');
const jurisdiccionSectionEdit = document.getElementById('jurisdiccionSectionEdit<?php echo $u['id']; ?>');
const empresaSectionEdit = document.getElementById('empresaSectionEdit<?php echo $u['id']; ?>');
const roleDescripcionEdit = document.getElementById('roleDescripcionEdit<?php echo $u['id']; ?>');
if (roleEdit) {
...
// Toggle jurisdiccion section
if (esOperador) {
jurisdiccionSectionEdit.classList.add('active');
jurisdiccionSectionEdit.querySelector('select').setAttribute('required', 'required');
} else {
jurisdiccionSectionEdit.classList.remove('active');
jurisdiccionSectionEdit.querySelector('select').removeAttribute('required');
}
// Toggle empresa section
if (esEmpresa) {
empresaSectionEdit.classList.add('active');
empresaSectionEdit.querySelector('select').setAttribute('required', 'required');
} else {
empresaSectionEdit.classList.remove('active');
empresaSectionEdit.querySelector('select').removeAttribute('required');
}
roleDescripcionEdit.textContent = selectedOption.dataset.descripcion || 'Acceso limitado';
});
}
})();
<?php endforeach; ?>
...
usuarioIdEliminar = this.dataset.id;
document.getElementById('usernameEliminar').textContent = this.dataset.username;
document.getElementById('idEliminar').value = usuarioIdEliminar;
new bootstrap.Modal(document.getElementById('modalEliminar')).show();
});
});
document.getElementById('confirmarEliminar')?.addEventListener('click', function() {
if (!usuarioIdEliminar) return;
// ✅ CORRECCIÓN M1: Incluir token CSRF en fetch de eliminación
fetch('usuarios.php', {
method: 'POST',
headers: {'Content-Type': 'application/x-www-form-urlencoded'},
body: `action=delete&id=${usuarioIdEliminar}&csrf_token=<?php echo $_SESSION['csrf_token']; ?>`
})
.then(r => r.json())
.then(data => {
if (data.success) {
