...
$stmt = $conn->prepare("SELECT id FROM pagos_servicios WHERE boleta_numero = ? AND activo = 1");
...
$stmt = $conn->prepare("SELECT * FROM pagos_servicios WHERE id = ? AND activo = 1");
$stmt->execute([$id]);
...
// ============================================================================
// 5. MANEJAR DESACTIVACIÓN DE PAGO (SOFT DELETE)
// ============================================================================
...
// UPDATE para preservar historial en lugar de DELETE
$stmt = $conn->prepare("UPDATE pagos_servicios SET activo = 0, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = ?");
$stmt->execute([$id]);
logAuditoria($conn, 'PAGO_DESACTIVADO', 'pagos_servicios', $id, [
'boleta' => $pago['boleta_numero'],
'empresa' => $pago['empresa_nombre'],
'monto' => $pago['monto_total'],
'motivo' => 'Desactivación lógica para preservación de auditoría'
], $user['id']);
$_SESSION['success'] = "<div class='alert alert-success'><i class='fas fa-check-circle me-2'></i>Pago desactivado correctamente (registro preservado para auditoría)</div>";
header('Location: pagos_servicios.php');
exit;
} catch (Exception $e) {
$_SESSION['error'] = "<strong>❌ Error:</strong> " . htmlspecialchars($e->getMessage());
...
$stmt = $conn->prepare("SELECT * FROM recordatorios_pagos WHERE id = ? AND activo = 1");
$stmt->execute([$id]);
$datos_antiguos = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$datos_antiguos) {
...
// ============================================================================
// 5.3 MANEJAR DESACTIVACIÓN DE RECORDATORIO (SOFT DELETE)
// ============================================================================
...
// UPDATE para preservar historial en lugar de DELETE
$stmt = $conn->prepare("UPDATE recordatorios_pagos SET activo = 0, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = ?");
$stmt->execute([$id]);
logAuditoria($conn, 'RECORDATORIO_DESACTIVADO', 'recordatorios_pagos', $id, [
'tipo' => $recordatorio['tipo_recordatorio'],
'empresa' => $recordatorio['empresa_nombre'],
'fecha' => $recordatorio['fecha_recordatorio'],
'motivo' => 'Desactivación lógica para preservación de auditoría'
], $user['id']);
$_SESSION['success'] = "<div class='alert alert-success'><i class='fas fa-check-circle me-2'></i>Recordatorio desactivado correctamente (registro preservado para auditoría)</div>";
header('Location: pagos_servicios.php#section-recordatorios');
exit;
} catch (Exception $e) {
$_SESSION['error'] = "<strong>❌ Error:</strong> " . htmlspecialchars($e->getMessage());
...
try {
$sql = "SELECT * FROM pagos_servicios WHERE activo = 1";
$params = [];
...
FROM pagos_servicios
WHERE activo = 1 AND estado IN ('pagado', 'parcial')
...
<form method="POST" action="" class="d-inline" onsubmit="return confirm('¿Desactivar este recordatorio? Se preservará para auditoría.');">
<input type="hidden" name="recordatorio_id" value="<?php echo $recordatorio['id']; ?>">
...
data-bs-target="#eliminarModal<?php echo $pago['id']; ?>" title="Desactivar (Soft Delete)">
<i class="fas fa-trash"></i>
</button>
</div>
</td>
...
<h5 class="modal-title">Desactivar Pago</h5>
<button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
</div>
<div class="modal-body">
<p><strong>¿Está seguro que desea desactivar este pago?</strong></p>
...
<i class="fas fa-info-circle"></i> El registro se preservará en la base de datos para cumplir con la Ley 19.549 de Procedimientos Administrativos y mantener la integridad de la auditoría.
</div>
</div>
<div class="modal-footer">
<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
<button type="submit" class="btn btn-danger">Desactivar Registro</button>
</div>
</form>
</div>
</div>
