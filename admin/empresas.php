...
// ============================================================================
// 10. MANEJAR DESACTIVACIÓN DE EMPRESA (DAR DE BAJA)
// ============================================================================
...
$stmt = $conn->prepare("DELETE FROM sucursales WHERE empresa_id = ?");
$stmt->execute([$id]);
...
$stmt = $conn->prepare("DELETE FROM servicios WHERE empresa_id = ?");
$stmt->execute([$id]);
...
$stmt = $conn->prepare("DELETE FROM recursos WHERE empresa_id = ?");
$stmt->execute([$id]);
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM personal WHERE empresa_id = ?");
$stmt->execute([$id]);
...
<p class='mb-0 text-warning'><small>Se eliminaron {$sucursales_eliminadas} sucursales, {$servicios_eliminados} servicios y {$recursos_eliminados} recursos. {$personal_desvinculado} personas desvinculadas.</small></p>
</div>
</div>
<button type='button' class='btn-close' data-bs-dismiss='alert'></button>
</div>
