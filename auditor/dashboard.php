...
$roles_permitidos = ['auditor'];
$rol_usuario = $auth->getCurrentUser()['rol'] ?? '';
if (!$auth->isLoggedIn() || !in_array($rol_usuario, $roles_permitidos)) {
...
// ? INSPECCIONES PROGRAMADAS (NUEVO)
...
// ?? DOCUMENTOS EMPRESAS (NUEVO - REEMPLAZA PRESENTISMO)
...
'descripcion' => "Hay {$total} empresa(s) inactivas",
...
'descripcion' => "Hay {$total} empresa(s) sin responsable",
...
'descripcion' => "Hay {$total} CUIT(s) duplicados",
'accion_url' => 'empresas.php'
];
}
} catch (Exception $e) { error_log("Error: " . $e->getMessage()); }
...
'prioridad' => 'alta',
'titulo' => 'Sucursales Pendientes',
'descripcion' => "Hay {$total} sucursal(es) pendientes",
...
'prioridad' => 'alta',
'titulo' => 'Aranceles Vencidos',
'descripcion' => "Hay {$total} sucursal(es) con arancel vencido",
...
'descripcion' => "Hay {$total} sucursal(es) rechazadas",
...
'descripcion' => "Hay {$total} registro(s) con documentación vencida",
'accion_url' => 'personal.php?filtro_vencimiento=vencido'
];
}
}
} catch (Exception $e) { error_log("Error: " . $e->getMessage()); }
...
'descripcion' => "Hay {$total} registro(s) por vencer en 30 días",
'accion_url' => 'personal.php?filtro_vencimiento=proximo'
];
}
}
} catch (Exception $e) { error_log("Error: " . $e->getMessage()); }
...
'descripcion' => "Hay {$total} registro(s) sin baja formal",
...
// =========================================================================
// ?? MÓDULO DOCUMENTOS EMPRESAS (NUEVO - REEMPLAZA PRESENTISMO)
// =========================================================================
try {
...
'descripcion' => "Hay {$total} documento(s) pendiente(s) de aprobación",
...
'descripcion' => "Hay {$total} documento(s) rechazado(s) en los últimos 7 días",
...
'prioridad' => 'baja',
'titulo' => 'Documentos Sin Motivación',
'descripcion' => "Hay {$total} documento(s) rechazados sin motivación",
'accion_url' => 'documentos_empresas.php?search_estado=rechazado'
];
}
}
...
'prioridad' => 'alta',
'titulo' => 'Recursos Pendientes',
'descripcion' => "Hay {$total} recurso(s) pendiente(s)",
...
'descripcion' => "Hay {$total} servicio(s) pendiente(s)",
...
'descripcion' => "Hay {$total} inspección(es) pendiente(s)",
...
// =========================================================================
// ? MÓDULO INSPECCIONES PROGRAMADAS (NUEVO)
// =========================================================================
try {
...
'descripcion' => "Hay {$total} inspección(es) programada(s) para hoy",
'accion_url' => 'inspecciones_programadas.php?filtro_fecha_desde=' . $hoy . '&filtro_fecha_hasta=' . $hoy
];
}
// Inspecciones programadas vencidas (pasadas y no realizadas)
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas WHERE fecha_programada < CURDATE() AND estado = 'pendiente'");
$total = $stmt->fetch()['total'] ?? 0;
if ($total > 0) {
$alertas[] = [
...
'descripcion' => "Hay {$total} inspección(es) programada(s) vencida(s)",
'accion_url' => 'inspecciones_programadas.php?filtro_estado=vencida'
];
}
// Inspecciones programadas pendientes (futuras)
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas WHERE fecha_programada > CURDATE() AND estado = 'pendiente'");
$total = $stmt->fetch()['total'] ?? 0;
if ($total > 0) {
$alertas[] = [
...
'descripcion' => "Hay {$total} inspección(es) programada(s) próximas",
...
'descripcion' => "Hay {$total} multa(s) pendientes de pago",
...
'auditoria_hoy' => 0,
// ? NUEVAS ESTADÍSTICAS PARA INSPECCIONES PROGRAMADAS
'inspecciones_programadas_total' => 0,
'inspecciones_programadas_pendientes' => 0,
'inspecciones_programadas_realizadas' => 0,
'inspecciones_programadas_hoy' => 0
];
try {
$stmt = $conn->query("SELECT COUNT(*) as total FROM empresas");
$stats['total_empresas'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM empresas WHERE activo = TRUE");
$stats['empresas_activas'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM personal");
$stats['total_personal'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM personal WHERE activo = TRUE");
$stats['personal_activo'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
...
// ? ESTADÍSTICAS DE INSPECCIONES PROGRAMADAS
if (tablaExiste($conn, 'inspecciones_programadas')) {
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas");
$stats['inspecciones_programadas_total'] = $stmt->fetch()['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas WHERE estado = 'pendiente'");
$stats['inspecciones_programadas_pendientes'] = $stmt->fetch()['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas WHERE estado = 'realizada'");
$stats['inspecciones_programadas_realizadas'] = $stmt->fetch()['total'];
$stmt = $conn->query("SELECT COUNT(*) as total FROM inspecciones_programadas WHERE fecha_programada = CURDATE() AND estado = 'pendiente'");
$stats['inspecciones_programadas_hoy'] = $stmt->fetch()['total'];
}
...
// Contar alertas por módulo (DOCUMENTOS en vez de PRESENTISMO)
$alertas_por_modulo = [
'empresas' => 0,
'sucursales' => 0,
'personal' => 0,
...
'servicios' => 0,
'inspecciones' => 0,
'inspecciones_programadas' => 0,
'documentos' => 0,
'otros' => 0
];
foreach ($alertas as $alerta) {
$modulo = obtenerModuloAlerta($alerta['tipo']);
if (isset($alertas_por_modulo[$modulo])) {
$alertas_por_modulo[$modulo]++;
}
}
// Log de auditoría
try {
...
.alertas-header .alertas-title {
display: flex;
align-items: center;
gap: 15px;
}
.alertas-header .alertas-title h2 {
font-weight: 800;
font-size: 1.5rem;
margin: 0;
}
.alertas-header .alertas-badge {
...
.alertas-header .toggle-icon {
font-size: 1.5rem;
transition: transform 0.3s ease;
}
.alertas-header.collapsed .toggle-icon {
...
.alertas-filters .filter-btn:hover {
border-color: #4361ee;
background: #ebf4ff;
}
.alertas-filters .filter-btn.active {
background: #4361ee;
border-color: #4361ee;
color: white;
}
.alertas-filters .filter-btn .badge-count {
...
.alerta-item .alerta-description {
color: #555;
margin-bottom: 15px;
line-height: 1.5;
}
.alerta-item .alerta-action {
...
.alerta-item .alerta-action:hover {
background: #3a0ca3;
...
.alertas-filters { flex-direction: column; }
.alertas-filters .filter-btn { width: 100%; justify-content: center; }
}
...
<body>
<?php $page_title = 'Dashboard Administrador'; include '../includes/header_auditor.php'; ?>
<div class="dashboard">
...
<?php echo $total_alertas; ?> alertas pendientes
</div>
</div>
</div>
...
<button class="filter-btn active" data-filter="all" onclick="filterAlertas('all')">
<i class="fas fa-list"></i> Todas
<span class="badge-count"><?php echo $total_alertas; ?></span>
</button>
<button class="filter-btn" data-filter="empresas" onclick="filterAlertas('empresas')">
<i class="fas fa-building"></i> Empresas
<span class="badge-count"><?php echo $alertas_por_modulo['empresas']; ?></span>
</button>
<button class="filter-btn" data-filter="sucursales" onclick="filterAlertas('sucursales')">
<i class="fas fa-store"></i> Sucursales
<span class="badge-count"><?php echo $alertas_por_modulo['sucursales']; ?></span>
</button>
<button class="filter-btn" data-filter="personal" onclick="filterAlertas('personal')">
<i class="fas fa-users"></i> Personal
<span class="badge-count"><?php echo $alertas_por_modulo['personal']; ?></span>
</button>
<button class="filter-btn" data-filter="recursos" onclick="filterAlertas('recursos')">
<i class="fas fa-boxes"></i> Recursos
<span class="badge-count"><?php echo $alertas_por_modulo['recursos']; ?></span>
</button>
<button class="filter-btn" data-filter="servicios" onclick="filterAlertas('servicios')">
<i class="fas fa-concierge-bell"></i> Servicios
<span class="badge-count"><?php echo $alertas_por_modulo['servicios']; ?></span>
</button>
<button class="filter-btn" data-filter="inspecciones" onclick="filterAlertas('inspecciones')">
<i class="fas fa-clipboard-check"></i> Inspecciones
<span class="badge-count"><?php echo $alertas_por_modulo['inspecciones']; ?></span>
</button>
<!-- ? NUEVO FILTRO PARA INSPECCIONES PROGRAMADAS -->
<button class="filter-btn" data-filter="inspecciones_programadas" onclick="filterAlertas('inspecciones_programadas')">
<i class="fas fa-calendar-check"></i> Programadas
<span class="badge-count"><?php echo $alertas_por_modulo['inspecciones_programadas']; ?></span>
</button>
<button class="filter-btn" data-filter="documentos" onclick="filterAlertas('documentos')">
<i class="fas fa-file-contract"></i> Documentos
<span class="badge-count"><?php echo $alertas_por_modulo['documentos']; ?></span>
</button>
<button class="filter-btn" data-filter="otros" onclick="filterAlertas('otros')">
<i class="fas fa-folder"></i> Otros
<span class="badge-count"><?php echo $alertas_por_modulo['otros']; ?></span>
</button>
</div>
...
?>
<div class="alerta-item <?php echo $alerta['prioridad']; ?>"
data-module="<?php echo $modulo; ?>"
style="display: none;">
<div class="alerta-header">
<div class="alerta-icon" style="background: <?php echo $color; ?>;">
<i class="fas fa-<?php echo $icono; ?>"></i>
</div>
<div class="alerta-title"><?php echo htmlspecialchars($alerta['titulo']); ?></div>
<span class="alerta-priority <?php echo $alerta['prioridad']; ?>">
<?php echo $alerta['prioridad']; ?>
</span>
</div>
...
<a href="<?php echo $alerta['accion_url']; ?>" class="alerta-action">
<i class="fas fa-arrow-right me-1"></i> Resolver Ahora
</a>
<?php endif; ?>
</div>
<?php endforeach; ?>
...
<p>El sistema no ha detectado ninguna situación que requiera atención inmediata.</p>
</div>
<?php endif; ?>
</div>
</div>
...
<span><?php echo $stats['empresas_activas']; ?> activas</span>
</div>
</div>
<div class="stat-card-modern stat-card-2">
<div class="stat-icon"><i class="fas fa-users"></i></div>
<div class="stat-value"><?php echo $stats['total_personal']; ?></div>
<div class="stat-label">Personal Registrado</div>
<div class="stat-trend up">
<i class="fas fa-arrow-up"></i>
<span><?php echo $stats['personal_activo']; ?> activos</span>
</div>
</div>
...
<div class="stat-trend up">
<i class="fas fa-arrow-up"></i>
<span><?php echo $stats['usuarios_activos']; ?> activos</span>
</div>
</div>
...
<div class="stat-trend <?php echo $total_alertas > 0 ? 'down' : 'up'; ?>">
<i class="fas fa-<?php echo $total_alertas > 0 ? 'exclamation-circle' : 'check-circle'; ?>"></i>
<span><?php echo $total_alertas > 0 ? 'Requieren atención' : 'Sin alertas'; ?></span>
</div>
</div>
<!-- ? NUEVA TARJETA: INSPECCIONES PROGRAMADAS PARA HOY -->
<?php if ($stats['inspecciones_programadas_total'] > 0): ?>
<div class="stat-card-modern stat-card-7">
<div class="stat-icon"><i class="fas fa-calendar-day"></i></div>
<div class="stat-value"><?php echo $stats['inspecciones_programadas_hoy']; ?></div>
<div class="stat-label">Inspecciones Hoy</div>
<div class="stat-trend <?php echo $stats['inspecciones_programadas_hoy'] > 0 ? 'down' : 'up'; ?>">
<i class="fas fa-<?php echo $stats['inspecciones_programadas_hoy'] > 0 ? 'calendar-check' : 'check-circle'; ?>"></i>
<span><?php echo $stats['inspecciones_programadas_pendientes']; ?> pendientes</span>
</div>
</div>
<?php endif; ?>
</div>
...
<a href="empresas.php" class="quick-action-card quick-action-1">
<div class="icon"><i class="fas fa-building"></i></div>
<div class="label">Gestionar Empresas</div>
</a>
<a href="personal.php" class="quick-action-card quick-action-2">
<div class="icon"><i class="fas fa-users"></i></div>
<div class="label">Gestionar Personal</div>
</a>
<a href="sucursales.php" class="quick-action-card quick-action-3">
<div class="icon"><i class="fas fa-store"></i></div>
<div class="label">Gestionar Sucursales</div>
</a>
<a href="inspecciones.php" class="quick-action-card quick-action-4">
<div class="icon"><i class="fas fa-clipboard-check"></i></div>
<div class="label">Inspecciones</div>
</a>
<!-- ? NUEVO ACCESO RÁPIDO: INSPECCIONES PROGRAMADAS -->
<a href="inspecciones_programadas.php" class="quick-action-card quick-action-7">
<div class="icon"><i class="fas fa-calendar-check"></i></div>
<div class="label">Inspecciones Programadas</div>
</a>
<a href="calendario_vencimientos.php" class="quick-action-card quick-action-8">
<div class="icon"><i class="fas fa-calendar-alt"></i></div>
<div class="label">Calendario de Vencimientos</div>
</a>
<a href="documentos_empresas.php" class="quick-action-card quick-action-5">
<div class="icon"><i class="fas fa-file-contract"></i></div>
<div class="label">Documentos Empresas</div>
</a>
<a href="auditoria.php" class="quick-action-card quick-action-6">
<div class="icon"><i class="fas fa-clipboard-list"></i></div>
<div class="label">Auditoría</div>
</a>
<a href="configuracion.php" class="quick-action-card quick-action-6">
...
document.querySelectorAll('.alertas-filters .filter-btn').forEach(btn => {
...
if (filter === 'all' || item.dataset.module === filter) {
