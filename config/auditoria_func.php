...
// ============================================================================
// 15. LIMPIAR AUDITORÍA ANTIGUA (Solo administradores) - MODIFICADO: UPDATE en lugar de DELETE
// ============================================================================
if (!function_exists('limpiarAuditoriaAntigua')) {
function limpiarAuditoriaAntigua($conn, $dias_a_conservar = 365, $usuario_id = null) {
try {
// Contar registros a archivar (excluyendo ya archivados)
$stmt = $conn->prepare("
...
$stmt->execute();
$total_a_archivar = $stmt->fetch()['total'];
if ($total_a_archivar > 0) {
// ARCHIVAR registros antiguos mediante UPDATE en lugar de DELETE para preservar historial
$stmt = $conn->prepare("
UPDATE auditoria
SET archivada = 1, 
    fecha_archivo = NOW(),
    motivo_archivo = 'Limpieza automática por antigüedad'
WHERE created_at < DATE_SUB(NOW(), INTERVAL :dias DAY)
AND (archivada IS NULL OR archivada = 0)
");
$stmt->bindValue(':dias', $dias_a_conservar, PDO::PARAM_INT);
...
// Log de la operación de archivo
logAuditoria($conn, 'AUDITORIA_ARCHIVADA', 'auditoria', null, [
...
'mensaje' => 'No hay registros antiguos para archivar'
];
} catch(PDOException $e) {
error_log("Error archivando auditoría antigua: " . $e->getMessage());
return [
'success' => false,
'error' => $e->getMessage()
];
